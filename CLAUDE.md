...
- **남은 TASK 를 묻거나 다음 작업을 추천할 때는 요약 HTML 문서를 함께 만들어 브라우저로 연다.** 대화 답변만으로 끝내지 않는다.
  - `html-report` 스킬로 작성하고 `docs/tasks/TASKS-remaining-<YYYY-MM-DD>.html` 에 저장한다.
  - 담을 것: 우선순위별 TASK 표(공수·상태·남은 일) · **판단 대기 항목** · 추천 1건과 그 근거.
  - **문서에 적힌 상태를 그대로 옮기지 말고 코드·데이터로 실제 상태를 확인한 뒤 적는다** — TASK 문서는 작성 시점 기록이라 낡아 있는 경우가 많다(실제로 TASK-020 의 확정 결함 5건 중 4건이 이미 해소돼 있었다).
