Shader "UI/Hologram Alpha Glow Only"
{
    Properties
    {
        ...
        _SilhouetteFill ("Silhouette Fill", Range(0, 1)) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float alpha = AlphaAt(IN.texcoord) * IN.color.a;
                float2 px = _MainTex_TexelSize.xy * _GlowSize;

                float a0 = AlphaAt(IN.texcoord + float2( px.x, 0.0));
                float a1 = AlphaAt(IN.texcoord + float2(-px.x, 0.0));
                float a2 = AlphaAt(IN.texcoord + float2(0.0,  px.y));
                float a3 = AlphaAt(IN.texcoord + float2(0.0, -px.y));
                float a4 = AlphaAt(IN.texcoord + float2( px.x,  px.y));
                float a5 = AlphaAt(IN.texcoord + float2(-px.x,  px.y));
                float a6 = AlphaAt(IN.texcoord + float2( px.x, -px.y));
                float a7 = AlphaAt(IN.texcoord + float2(-px.x, -px.y));
                ...
                float silhouette = alpha * _SilhouetteFill;
                float glowMask = saturate(outerHalo * _OuterGlow + innerRim * _InnerGlow + silhouette);
