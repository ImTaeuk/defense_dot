Shader "UI/Hologram/Foil Tinted"
{
    Properties
    {
        ...
        _RainbowScale ("Color Cycle Scale", Range(1, 20)) = 8
        _RainbowSpeed ("Color Cycle Speed", Range(0, 3)) = 0.35
        _FoilBandScale ("Foil Band Scale", Range(0.3, 12)) = 3.0

        _FoilColorA ("Foil Color A", Color) = (0.2, 0.5, 1, 1)
        _FoilColorB ("Foil Color B", Color) = (0.7, 0.9, 1, 1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                // 무지개 대신 2색 틴트 — 등급별 색 계열
                float colorMix = 0.5 + 0.5 * sin(rainbowPhase);
                float3 foilColor = lerp(_FoilColorA.rgb, _FoilColorB.rgb, colorMix);
