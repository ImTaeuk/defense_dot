...
//   IAbilityCommandTarget(TowerAbilitySystem 구현)이고, 골드 증감은 EconomyModel 이 한다.
//
// [왜 안 쓰이나]
...
namespace DefenseDot.Systems.Economy
{
    ...
    public sealed class AbilityUpgradeSystem
    {
        private readonly IAbilityCommandTarget abilityTarget;   // 능력 레벨업/삭제 명령 대상
        ...
        public AbilityUpgradeSystem(IAbilityCommandTarget abilityTarget, EconomyModel economy, AbilityUpgradeConfig config)
        {
        ...
        public bool TryUpgrade(AbilityInstance ability)
        {
            ...
            abilityTarget.LevelUpAbility(ability);
            return true;
        }
        ...
        public void Dismiss(AbilityInstance ability)
        {
            ...
            abilityTarget.RemoveAbility(ability);
        }
    }
}
