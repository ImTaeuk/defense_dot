// ModeBootstrap 인스펙터에 카메라 각도 프리뷰 버튼을 붙인다
using UnityEngine;
using UnityEditor;
...
{
    /// <summary>
    /// ModeBootstrap 인스펙터에 카메라 각도 미리보기 버튼을 추가합니다.
    /// 씬을 편집할 때 그 씬이 열려 있으므로, Config 소유 주체가 프리뷰의 주체가 됩니다.
    /// </summary>
    [CustomEditor(typeof(ModeBootstrap), true)]
    public class ModeBootstrapEditor : UnityEditor.Editor
    {
        ...
        private void MoveSceneViewToCameraPose()
        {
            ...
            if (config == null)
            {
                Debug.LogError("[ModeBootstrapEditor] cameraConfig가 할당되지 않았습니다.", target);
                return;
            }
            ...
            if (view == null)
            {
                Debug.LogError("[ModeBootstrapEditor] 활성 씬 뷰가 없습니다.", target);
                return;
            }
