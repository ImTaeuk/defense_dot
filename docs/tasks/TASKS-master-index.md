...
|---|---|---|---|
| 보스 (N라운드마다) | `isBoss`(100), `bossIntro` | ❌ 없음 · **리소스 대기**(보스 에셋 없음) | A6 |
| 계보 공명 (SR+SSR 조합 시너지, v608) | `LINEAGE_RESONANCE` (23계보×5축) | ❌ 없음 · **A7 가챠(캐릭터) 선행 필요** → 메타층 묶음 | A7-9 (구 A8) |
| HUD/UI 리스킨 (메카닉 HUD·능력 도크) | v575~v606, `ABILITY_DOCK` | 🔶 부분(ArenaHud) · **리소스 대기**(UI 에셋 없음) | B |
...
- **A6 보스 · B HUD** — 페이싱·체감 향상. 중~고비용. **현재 리소스(보스·UI 에셋) 부재로 착수 대기.**
- **메타층(A7)** — 볼륨이 커 **별도 TASK로 분리 예정**(2026-07-20 결정). Arena 방향이 (a)메타 진행형으로 확정되면 우선도 ★★로 상승. 아니면 후순위.
- **계보 공명(구 A8 → A7-9)** — 캐릭터 SR+SSR 조합 각성 이벤트. 우리 능력 합성(A5-1)과 다른 층으로, **A7 가챠(캐릭터) 선행 필요** → 메타층 묶음에 편입.
...
| [021](active/TASK-021-fusion-lineage-ui-readability.md) | 합성 계보 UI 가독성 | 높음 | S~M | 합성 카드·재료·소진이 플레이어에게 전달 안 됨 |
| [020](active/TASK-020-play-loop-completeness-audit.md) | 플레이 루프 완성도 감사 | 높음 | M | 확정 결함 4건 중 시전 병목 해결·나머지 3건 잔여 |
...
| [022](active/TASK-022-meta-progression-layer.md) | 메타 진행층 (런 밖 성장, 마스터) | 보류 | **XL** | A7 전체(가챠·유물·승천·행성·계급·업적·기록·랭킹·계보공명) 분리 |
| [011](active/TASK-011-meta-shop.md) | 메타 상점 | 낮음 | XL* | 022 A7-3으로 흡수 |
...
- **2026-07-20** — **공격 체계 재설계·플레이 루프 감사.** 시전 애니가 발사를 막아 능력을 모아도 화력이 안 늘던 구조를 해소 — 능력을 발동 시점 기준 주축·동반·자율 3타입으로 나누고 발사 주기를 `CoreWeapon` 이 단독 소유하도록 리팩토링(테스트 16건). 능력 에셋 4종을 새 타입으로 이관, 데미지를 1타 10·적 체력 30 으로 정규화(오비탈 계열만 6타 기준). 계보 합성이 주축을 소진할 때 결과도 주축이어야 성립하는 규칙과 주축 중복 획득 차단을 추가해 정점→하위 다운그레이드 경로 차단. 6커밋. **TASK-020**(플레이 루프 완성도 감사) 신규 등록 — 패배 도달 불가·패시브 보정 생산자 부재·스폰 플래그 누수 3건 잔여. **TASK-021**(합성 계보 UI 가독성) 신규 등록 — 시스템은 작동하나 합성 카드·재료·소진이 플레이어에게 전달되지 않음.
