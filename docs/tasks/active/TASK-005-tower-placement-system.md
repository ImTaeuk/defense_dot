...
### C. Skill 시스템 (원작 능력 강화 — 공격 방식 분리)
> 공격 방식을 **Skill** 이라는 이름의 능력으로 분리한다. 원작의 능력 강화(능력 카드·강화) 시스템에 대응하며, 타워가 Skill 을 장착해 공격한다. 현재 단일/범위/투사체는 `TowerActor` 의 디버그 토글(TASK-004)로 임시 구현된 상태.
- [ ] **C-1.** Skill 데이터·획득(드랍/레벨업)·장착(타워에 Skill 부착) — **TASK-004 디버그 공격 behavior 3종(단일/범위/투사체)을 이 Skill 시스템으로 대체·삭제**
- [ ] **C-2.** Skill 강화/조합 (원작 능력 카드 합성·레벨업)
- [ ] **C-3.** `aoeRadius` 등 디버그용 `TowerData` 임시 필드를 Skill 데이터로 이관
